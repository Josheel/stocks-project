...
import { fetchStockOverview, StockOverviewResponse} from '../services/alpha_vantage_api';
...
const StockDetails: React.FC = () => {
  ...
  useEffect(() => {
    if (symbol) {
      fetchStockOverview(symbol, 'details').then((data) => {
        console.log(data);
      ...
      {symbol && stockOverview.Details ? (
        <div className="bg-[#0f1115] text-gray-100 p-6 rounded-lg">
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-4 gap-x-8">
            ...
            <li className="flex justify-between border-b border-dashed border-gray-700 pb-1">
              <span className="text-gray-400 font-semibold">Ex Dividend:</span>
